...
### 003_posts_post_translation_entries_column.sql **(required for blog edit)**
...
### 005_post_translation_entries_table.sql **(fixes "relation post_translation_entries does not exist")**

- Creates the **`post_translation_entries`** table and **`enum_post_translation_entries_locale`** enum expected by Payload’s `post-translation-entries` collection. **If you see "relation post_translation_entries does not exist" on the dashboard translations page, run this migration.** Idempotent; safe if the table/enum already exist.
