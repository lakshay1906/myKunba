...
export function convertHtmlToLexicalWithParser(html: string): PayloadLexicalContent {
  ...
  function processNode(
    node: Node,
    inheritedFormat = 0,
  ): (LexicalTextNode | LexicalElementNode)[] {
    ...
    if (node.nodeType === 3) {
      ...
      switch (tagName) {
        ...
        case 'p':
          ...
          if (pInline.length > 0) {
            // Keep the paragraph if ANY inline child carries content:
            //  - a text node with non-whitespace text, OR
            //  - any non-text inline element (link, formatting span, etc.)
            const hasMeaningfulContent = pInline.some((child) => {
              if (child.type === 'text') {
                return (child as LexicalTextNode).text?.trim().length > 0
              }
              return true
            })
        ...
        case 'a': {
          ...
          result.push({
            ...
            version: 1,
            children:
              elementChildren.length > 0 ? elementChildren : [createTextNode(href, inheritedFormat)],
            direction: 'ltr',
            format: '',
            indent: 0,
            url: href,
