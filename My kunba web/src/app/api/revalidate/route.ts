 ...
 * Example: POST /api/revalidate with body { "path": "/blog/my-post-slug", "secret": "your-secret" }
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof path !== 'string' || !path.startsWith('/') || path.includes('..')) {
      return NextResponse.json(
        { message: 'Invalid path. Use a path like /blog, /blog/my-slug, /category/tech, /author/1, /' },
        { status: 400 },
      )
    }
    ...
    return NextResponse.json(
      { message: 'Path not supported for revalidation. Use /blog/slug, /category/slug, /author/id, /, or /blog' },
      { status: 400 },
    )
  } catch (error) {
